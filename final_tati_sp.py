...
from pyspark.sql.functions import sin, cos, asin, sqrt
from math import pi
...
config = {'version': 'v1',
 'config': {'visState': {'filters': [],
   'layers': [{'id': 'mgi0vy',
     'type': 'point',
     'config': {'dataId': 'Realty from cian, avito',
      'label': 'Realty',
      'color': [130, 154, 227],
    ...
    {'id': 'y3vlf09',
     ...
     'visualChannels': {'colorField': None,
      ...
      'sizeField': None,
      'sizeScale': 'linear'}}],
   'interactionConfig': {'tooltip': {'fieldsToShow': {'Realty from cian, avito': [{'name': 'url',
        'format': None},
...
def send_map_to_chat(chats, path):
    ...
    for chat in chats:
        f = open(path,"rb")
        bot.send_document(MY_CHAT_ID, f)


def make_map(realty_loaded, wb_pvz_loaded):
    Map_new = KeplerGl(height=1000, config=config)
    Map_new.add_data(realty_loaded[['url', 'square', 'loaded_offices_cnt', 'phone', 'lat', 'lon']], name='Realty from cian, avito')
    ...
    Map_new.save_to_html(file_name='/opt/hadoop/airflow/dags/obarskaya/cian_realty_map_' + dt + '.html', config=config)
    working_chats = define_working_chat()
    send_map_to_chat(working_chats, '/opt/hadoop/airflow/dags/obarskaya/cian_realty_map_' + dt + '.html')
...
def _parsing_of_wb_points():
    ...
    if all_pvz_data.shape[0] > 0 and all_pvz_data[all_pvz_data['deliveryPrice'] > 0].shape[0] > 0:
 ...
 #       wb_pvz.repartition(1).write.mode('overwrite').parquet('/user/obarskaya_ts/af/wb/')
    else:
        logging.info('No loaded wb points')
...
#    cian_df.repartition(1).write.mode('overwrite').parquet('/user/obarskaya_ts/af/cian/')
...
def _reading_hdfs_filtring():
    ...
    wb_pvz = spark.read.option('header', 'true').parquet('/user/obarskaya_ts/af/wb/')
    realty_df = spark.read.option('header', 'true').parquet('/user/obarskaya_ts/af/cian/')
    w = Window().partitionBy('id')
    wb_loaded = wb_pvz.withColumnRenamed('lat','wlat').withColumnRenamed('lon','wlon')
    merged_df = realty_df.select('id', 'lat', 'lon')
    merged_df = merged_df.withColumnRenamed('lat','rlat').withColumnRenamed('lon','rlon')
    merged_df = merged_df.select('id', 'rlat', 'rlon').crossJoin(wb_loaded.select('office_id', 'deliveryPrice', 'wlat', 'wlon'))
    merged_df = merged_df.select(col('id'), col("rlat").cast("float"), col("rlon").cast("float"), col("deliveryPrice").cast("float"), col('office_id'), col("wlat").cast("float"), col("wlon").cast("float"))
    merged_df = merged_df.withColumn('rlat_rad', col('rlat') * pi / 180)\
        .withColumn('rlon_rad', col('rlon') * pi / 180)\
        .withColumn('wlat_rad', col('wlat') * pi / 180)\
        .withColumn('wlon_rad', col('wlon') * pi / 180)\
        .withColumn('diff_lat', col('rlat_rad') - col('wlat_rad'))\
        .withColumn('diff_lon', col('rlon_rad') - col('wlon_rad'))\
        .withColumn('first', sin(col('diff_lat')/2) ** 2)\
        .withColumn('second', cos(col('rlat')))\
        .withColumn('third', cos(col('wlat')))\
        .withColumn('fourth', sin(col('diff_lon')/ 2) ** 2)\
        .withColumn('distance', 2 * 6367444.6571225 * asin(sqrt(col('first') + col('second') * col('third') * col('fourth'))))\
        .filter('distance < 3500 and distance > 100')\
        .select('id', 'office_id','distance', 'deliveryPrice')
    merged_df = merged_df.withColumn('mean_delivery_price', mean('deliveryPrice').over(w))\
        .withColumn('loaded_offices_cnt', count('office_id').over(w))\
        .select('id', 'office_id', 'distance', 'deliveryPrice', 'loaded_offices_cnt', 'mean_delivery_price')
    realty_loaded = realty_df.join(merged_df, realty_df.id == merged_df.id, how='inner')
    realty_loaded = realty_loaded.filter(col('deliveryPrice') > 100)
    logging.info('Everything is done.')
   # logging.info('Final DF after filtring:', realty_loaded.shape)
    wb_pvz = wb_pvz.toPandas()
    realty_loaded = realty_loaded.toPandas()
    logging.info('Realty DF Loaded of shape', realty_loaded.shape)
