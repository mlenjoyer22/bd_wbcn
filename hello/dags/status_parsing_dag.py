...
POSTGRES_DB = URL.create(drivername='postgresql+psycopg2',
                         username="postgres", password="postgres",
                         host="postgres-db", database="postgres_db")
...
def write_pvz_info(all_pvz_data, engine):
    ...
    engine.execute(text(
        "DROP TABLE IF EXISTS public.wb_points; CREATE TABLE IF NOT EXISTS public.wb_points ("
        "office_id integer, "
        "address text, "
        "fittingRooms text, "
        "workTime text, "
    ...
    new_pvz_data.columns = new_pvz_data.columns.str.lower()
    logging.info('Inserting rows with columns %s', new_pvz_data.columns)
...
def write_pvz_status(all_pvz_data, engine):
    ...
    engine.execute(text(
        "DROP TABLE IF EXISTS public.pvz_status; CREATE TABLE IF NOT EXISTS public.pvz_status ("
        "office_id integer, "
        "status text, "
        "deliveryPrice float, "
        "dt timestamp"
    ...
    pvz_status.columns = pvz_status.columns.str.lower()
